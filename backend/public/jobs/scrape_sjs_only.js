...
async function scrapeSJS() {
  if (!await checkDatabaseConnection()) {
    console.error('Cannot proceed without database connection');
    process.exit(1);
  }

  const maxPages = 250; // Number of pages to scrape for SJS
  let newCount = 0;
  let updateCount = 0;
  let errorCount = 0;
  let scrapedCount = 0;

  console.log('Starting SolarisJapan scrape...');
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      const prods = await scrapeJS.scrapeJSVari(page);
