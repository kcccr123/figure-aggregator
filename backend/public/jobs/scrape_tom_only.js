...
const scrapeingTOM = require('../scrappers/stom.js');
...
async function scrapeTOM() {
  ...
  const maxPages = 250; // Number of pages to scrape for TOM
  let newCount = 0;
  let updateCount = 0;
  let errorCount = 0;
  let scrapedCount = 0;

  console.log('Starting Tokyo Otaku Mode scrape...');
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      const prods = await scrapeingTOM.scrapeTOMVari(page);
