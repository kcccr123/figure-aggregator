...
const scrapingAnimota = require('../scrappers/animota.js');
...
async function scrapeAnimota() {
  ...
  const maxPages = 20; // Number of pages to scrape for Animota
  let newCount = 0;
  let updateCount = 0;
  let errorCount = 0;
  let scrapedCount = 0;

  console.log('Starting Animota scrape...');
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      const prods = await scrapingAnimota.scrapeAnimotaVari(page);
